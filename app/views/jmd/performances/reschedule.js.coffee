...
$('#<%= @new_day ? @new_day.to_s : "unscheduled" %> .draggable-performances').html(
  "<%= escape_javascript(render partial: 'draggable_performance',
       collection: @performances.on_date(@new_day).at_stage_venue(@new_stage_venue.try(:id)).browsing_order, as: :performance) %>"
)
$('#<%= @old_day ? @old_day.to_s : "unscheduled" %> .draggable-performances').html(
  "<%= escape_javascript(render partial: 'draggable_performance',
       collection: @performances.on_date(@old_day).at_stage_venue(@old_stage_venue.try(:id)).browsing_order, as: :performance) %>"
)
